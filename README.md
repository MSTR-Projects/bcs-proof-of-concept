...
- **Seeds** Hoveniersbedrijf De Groene Linde (20 medewerkers) with the serie "Maandafsluiting februari 2026": five controles (Loonstrokencontrole, Loonjournaal maandcontrole, Medewerkersbestand aansluiting, Loonaangifte controle, Reserveringencontrole), one global value group (CAO Hoveniersbedrijf 2026) and 25 sample files. Rules reference the seeded global values, spreadsheet totals reconcile with PDF totals, and PDF field regions are derived with pdfplumber so extraction works like a hand-drawn template.
...
Generated sample files are copied to `sample_files/hoveniersbedrijf-de-groene-linde/` so they can be re-uploaded manually in the Run dialogs. `sample_pdfs/` contains additional standalone rapport-PDFs (verwerkingssignalen, loonaangifte, TWK, etc.) generated by `backend/generate_sample_controle_pdfs.py`.
...
4. Have the `sample_files/hoveniersbedrijf-de-groene-linde/` folder ready in a Finder/Explorer window for drag-and-drop.
...
4. Select **all files** in `sample_files/hoveniersbedrijf-de-groene-linde/` **except** `Loonstrook_2026-02_50114_correctie.pdf` (24 files) and drag them into the upload zone in one go.
5. Assign from the pool: all 20 loonstroken onto the **Loonstrook** slot (stap 1), the journaal, xlsx, aangifte and reserveringen PDFs onto their own slots. Every slot needs a file before the run button enables.
...
2. Upload the same 24 files **plus** `Loonstrook_2026-02_50114_correctie.pdf` (25 total); assign the 21 loonstroken to the Loonstrook slot and the rest as before.
